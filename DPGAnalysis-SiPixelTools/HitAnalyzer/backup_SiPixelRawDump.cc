...
//  TH1D *htimeoutFed, *hoverflowFed, *hnorFed, *heneFed, *hpixFed;
  ...
  TH2D *hFEDChannel[139], *eFEDChannel[139], *sumhFED, *sumeFED, *eFEDType[139], *sumType, *hModule;
  TH1F *PKAMCount;
...
void SiPixelRawDump::beginJob() {
  ...
  for (int i = 0; i < 139; i++){
    ...
    hFEDChannel[i] = hits.make<TH2D>(label_char2, title_char2, 48, 0., 48., 600, -0.5, 599.5);
    ...
    label_char2 = labels2.c_str();
    title_char2 = titles2.c_str();
    eFEDChannel[i] = hits.make<TH2D>(label_char2, title_char2, 48, 0., 48., 5, 0., 5.);
    ...
    label_char2 = labels2.c_str();
    title_char2 = titles2.c_str();
    eFEDType[i] = hits.make<TH2D>(label_char2, title_char2, 48, 0., 48., 17, 1., 18.);
  ...
  PKAMCount = pkam.make<TH1F>("PKAMCount", "Number of PKAMs in Each Layer 1 Channel", lyr1.size(), 0, lyr1.size());
  PKAMCount->GetXaxis()->SetTitle("Channel");
	...
	if(printLocal>3) cout<<" "<<num<<" "<<hex<<w<<dec<<endl;
        ...
        if (plotRoc <= 16) { 
          ...
          if (((w&errorMask) == trailError) && ((w&fsmMask) == PKAMreset) && (channelWords.size() > 200)){
            ...
            if (layer < 5) {
              cout << "We got us a PKAM bois in Event " << event << ", Layer " << layer << ", FED " << fedId << ", Channel " << PKAMChannel  << endl;
            } else {
              cout << "We got us a PKAM bois in Event " << event << ", FPix, FED " << fedId << ", Channel " << PKAMChannel  << endl;
            }
            ...
            for (unsigned int i=0;i < channelWords.size(); i++) {
              ...
              if (PKAMRoc <= 16) {  
                if (layer == 1) {
                  ...
                  //row = 80 - ((pix/16)*3 + (pix%16)/2 - (pix/256)*30);
                  //col = ((dcol/16)*6 + (dcol%16))*2 + (pix%2);
                  row = pix;
                  col = dcol;
                }
                else {
                  ...
                  //cout << "pix = 0x" << hex << pix << ", dcol = 0x" << hex << dcol << endl;
                  ...
                  //col_buff = dcol % 6; 
                  //col_buff_6 = (dcol - col_buff)/6;
                  //col_new = (col_buff_6 << 3) | col_buff;
                  ...
                  //pix_buff_36 = (pix << 6)/36;
                  //pix_buff_6 = ((pix-pix_buff_6)<<3)/6;
                  //pix_buff = pix - pix_buff - pix_buff_6; 
                  //pix_buff = pix%6;
                  //pix_buff_6 = (pix - pix_buff)/6;
                  //pix_buff_36 = (pix - pix_buff - pix_buff_6)/36;
                  //pix_new = (pix_buff_36 << 6) | (pix_buff_6 << 3) | (pix_buff);
 
                  //int pix2=(((pix & 0x1C0) >> 6)*36) + (((pix & 0x38) >> 3)*6) + (pix & 0x7);
 
                  //row = 80 - ((pix_new/8)*3 + (pix_new%8)/2 - (pix_new/64)*6);
                  //col = ((col_new/8)*12 + (col_new%8))*2 + (pix%2);
                  
                  row = 80 - (pix/2);
                  col = dcol*2 + pix%2;

                  //cout << "FED " << fedId << ", Channel " << PKAMChannel << ", ROC " << PKAMRoc << " has dcol " << dcol << " -> " << col << " -> " << (PKAMRoc-1)*52+col << endl; 
                  //cout << "FED " << fedId << ", Channel " << PKAMChannel << ", ROC " << PKAMRoc << " has pix " << pix << " -> " << pix_new << " -> " << pix_buff_36 << pix_buff_6 << pix_buff << " -> " << row << " -> " << abs((PKAMRoc/9)*160 - row) << endl;
                }

                if (row > 80 || row < 0 || col < 0 || col > 51) {
                  cout << "Illegal row/col in FED " << fedId << ", Channel " << PKAMChannel << ", row " << row << ", column " << col << endl;
                }
                ...
                //cout << "We filling this fam in Event " << PKAMEvent << ", FED " << fedId << ", Channel " << PKAMChannel << endl;
              }
            }
          }
