        ...
        if (plotChannel != lastChannel) {
          ...
          if (fedId > 1295) {
            ...
            for (unsigned int it = 0; it < map.size(); it++) {
              if (map[it][1] == fedId && map[it][2] == plotChannel) {
                layer = map[it][0];
                break;
              }
            }
          ...
          if (col_max > 60) {
            cout << "We gots some noise in Layer " << layer << ", FED " << fedId << ", Channel " << plotChannel << " with " << col_max << " hits" << endl;
            col_max = 0;           
